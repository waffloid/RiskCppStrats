...
static void test_stubs_return_empty() {
    Graph graph = build_path(3, 1.0f);
    GameConfig config{};
    std::vector<NodeData> nodes(3);

    BuildPlan mcmc = economy_solver_mcmc(graph, nodes, 0, config);
    assert(mcmc.steps.empty());

    BuildPlan bb = economy_solver_branch_bound(graph, nodes, 0, config);
    assert(bb.steps.empty());

    std::printf("test_stubs_return_empty: PASS\n");
}
